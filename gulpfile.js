...
function moveUnchangedFiles(done = Function.prototype) {
  pump(
    [
      gulp.src("src/bin/*"),
      gulpif(doVerbose, debug({ title: "moving ./bin/*:" })),
      gulp.dest("dist/bin")
    ],
    function cb(er) {
      if (er) {
...
function processPrivateJS(done = Function.prototype) {
  pump(
    [
      ...
      gulpif(doVerbose, debug({ title: "processing private js:" })),
      gulpif(doMinify, uglify({ toplevel: true, ecma: 6 })),
      gulp.dest("dist")
    ],
    function cb(er) {
      if (er) {
...
//convert index.pug etc. to html, keep error.pug as pug, minify the html.
function processViews(done = Function.prototype) {
  pump(
    [
      ...
      gulpif(doVerbose, debug({ title: "processing pug views:" })),
      gulpif(
        function({path}) {
          return !path.endsWith("error.pug");
        },
        pug(),
        gulpif(
          doMinify,
...
function processPublicJS(done = Function.prototype) {
  ...
  pump(
    [
      ...
      gulpif(doVerbose, debug({ title: "processing public js:" })),
      ...
      gulpif(doMinify, uglify({ toplevel: true, ecma: 5 })),
      gulp.dest("temp")
    ],
    function cb(er) {
      if (er) {
...
function processCSS(done = Function.prototype) {
  pump(
    [
      ...
      gulpif(doVerbose, debug({ title: "processing public CSS:" })),
      gulpif(doMinify, cleanCSS()),
...
function processImages(done = Function.prototype) {
  pump(
    [
      ...
      gulpif(doVerbose, debug({ title: "processing images:" })),
      gulpif(
        doImages,
        image({
          guetzli: false
